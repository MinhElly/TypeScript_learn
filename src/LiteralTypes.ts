...
function navigateRoute(route: string) {
  if (!isValidRoute(route)) {
    throw new Error(
      `Invalid route: ${route}. Must be a valid direction-distance combination.`,
    );
  }
...
function isValidApiRequest(request: string): boolean {
  const [method = "", route = ""] = request.split(" ");

  // Check if method is valid
  if (!httpMethods.includes(method)) return false;
